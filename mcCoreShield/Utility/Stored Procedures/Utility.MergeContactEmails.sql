CREATE PROCEDURE Utility.MergeContactEmails ( @MasterContactID   AS UNIQUEIDENTIFIER
...
************************************************************************************************************************************
  ...
  Procedure:    Utility.MergeContactEmails
     Author:    Chris Carson
    Purpose:    Merges records on dbo.ContactEmails for two coreShield Contacts
...
BEGIN TRY
    ...
    DECLARE @transactionStarted AS BIT
          , @tableName          AS NVARCHAR (20) = 'mc_contact_emails' ;
...
/**/      , @codeBlockDesc  = @codeBlockDesc03 ; -- INSERT partitioned record keys into temp storage
      ...
      WITH  records AS (
            SELECT  recordID    = c.ID
                  , recordKey   = ISNULL( email, 'xx' )
                  , systemID    = transitionSystemsID
                  , legacyID    = legacyID
              FROM  dbo.ContactEmails           AS c
