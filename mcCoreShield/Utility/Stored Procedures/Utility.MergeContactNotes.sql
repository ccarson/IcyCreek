CREATE PROCEDURE Utility.MergeContactNotes ( @MasterContactID   AS UNIQUEIDENTIFIER
...
************************************************************************************************************************************
  ...
  Procedure:    Utility.MergeContactNotes
     Author:    Chris Carson
    Purpose:    Merges records on dbo.ContactNotes for two coreShield Contacts
...
BEGIN TRY
    ...
    DECLARE @transactionStarted AS BIT
          , @tableName          AS NVARCHAR (20) = 'mc_contact_notes' ;
...
/**/      , @codeBlockDesc  = @codeBlockDesc03 ; -- INSERT partitioned record keys into temp storage
      ...
      WITH  records AS (
            SELECT  recordID    = c.ID
                  , recordKey   = ISNULL( notes, 'xx' )
                  , systemID    = transitionSystemsID
                  , legacyID    = legacyID
              FROM  dbo.ContactNotes            AS c
