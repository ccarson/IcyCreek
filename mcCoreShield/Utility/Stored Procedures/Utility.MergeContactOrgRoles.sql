CREATE PROCEDURE Utility.MergeContactOrgRoles ( @MasterContactID   AS UNIQUEIDENTIFIER
...
************************************************************************************************************************************
  ...
  Procedure:    Utility.MergeContactOrgRoles
     Author:    Chris Carson
    Purpose:    Merges records on dbo.ContactOrgRoles for two coreShield Contacts
      ...
      WITH  records AS (
            SELECT  recordID    = c.ID
                  , recordKey   = CAST( c.organizationsID AS NVARCHAR(50) ) + '|' +
                                  ...
                                  CAST( c.RolesID AS NVARCHAR(50) )
                  , systemID    = s.systemID
                  , legacyID    = s.mc_contact_orgrolesID
              FROM  dbo.ContactOrgRoleSystems   AS s
        INNER JOIN  dbo.ContactOrgRoles         AS c ON c.ID = s.ID
...
/**/      , @codeBlockDesc  = @codeBlockDesc05 ; -- DELETE portal records for some duplicates
      ...
      WITH  duplicates AS (
            ...
            records AS (
            SELECT  id, systemID, createdOn, createdBy, updatedOn, updatedBy, mc_contact_orgrolesID
              FROM  dbo.ContactOrgRoleSystems AS s
...
/**/      , @codeBlockDesc  = @codeBlockDesc09 ; -- create new portal records showing all records for all portals
    ...
    SELECT  @legacyID = MAX( mc_contact_orgrolesID )
      FROM  dbo.ContactOrgRoleSystems ;
    ...
    SELECT  ID                      = masterID
          ...
          , updatedOn               = NULL
          , updatedBy               = NULL
          , mc_contact_orgrolesID   = @legacyID + ROW_NUMBER() OVER ( ORDER BY ( SELECT NULL ) )
      FROM  newRecords AS r
     WHERE  NOT EXISTS ( SELECT 1 FROM dbo.ContactOrgRoleSystems AS s
