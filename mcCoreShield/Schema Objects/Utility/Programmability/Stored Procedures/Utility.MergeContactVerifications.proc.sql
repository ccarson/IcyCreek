CREATE PROCEDURE Utility.MergeContactVerifications ( @MasterContactID   AS UNIQUEIDENTIFIER
...
************************************************************************************************************************************
  ...
  Procedure:    Utility.MergeContactVerifications
     Author:    Chris Carson
    Purpose:    Merges records on dbo.ContactVerifications for two coreShield Contacts
...
BEGIN TRY
    ...
    DECLARE @transactionStarted AS BIT
          , @tableName          AS NVARCHAR (20) = 'mc_contact_verification' ;
...
/**/      , @codeBlockDesc  = @codeBlockDesc03 ; -- INSERT partitioned record keys into temp storage
      ...
      WITH  records AS (
            SELECT  recordID    = c.ID
                  , recordKey   = ISNULL( CAST( verified_by AS NVARCHAR(50) ), 'xx' )
                  , systemID    = transitionSystemsID
                  , legacyID    = legacyID
              FROM  dbo.ContactVerifications    AS c
...
/**/      , @codeBlockDesc  = @codeBlockDesc07 ; -- DELETE core records for modified duplicates
    ...
    DELETE  dbo.ContactAddresses
      FROM  dbo.ContactAddresses AS c
