...
BEGIN
	...
	BEGIN 
		...
		INNER JOIN dbo.ContactSystems AS tc1 ON i.user_id = tc1.mc_contactID  AND tc1.systemID = @systemID
		INNER JOIN dbo.ContactSystems AS tc2 ON i.admin_id = tc2.mc_contactID AND tc2.systemID = @systemID;
	...
	BEGIN 
		...
		INNER JOIN dbo.ContactNotes as cn ON cn.id = tn.id
			AND tn.transitionSystemsID = @systemID
		INNER JOIN dbo.ContactSystems AS tc1 ON u.user_id = tc1.mc_contactID  AND tc1.systemID = @systemID
		INNER JOIN dbo.ContactSystems AS tc2 ON u.admin_id = tc2.mc_contactID AND tc2.systemID = @systemID;
