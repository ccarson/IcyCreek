...
BEGIN
	...
	BEGIN 
		...
		INNER JOIN dbo.ContactSystems AS c1 ON #inserts.user_id = c1.mc_contactID 
			AND c1.systemID = @systemID
		INNER JOIN dbo.ContactSystems AS c2 ON #inserts.verified_by = c2.mc_contactID 
	...
	BEGIN 
		...
		INNER JOIN dbo.ContactVerifications AS cv ON v.id = cv.id
			AND	v.transitionSystemsID = @systemID
		INNER JOIN dbo.ContactSystems AS c ON #updates.verified_by = c.mc_contactID 
