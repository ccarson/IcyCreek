CREATE PROCEDURE dbo.mc_contact_orgrolesDELETE ( @systemID     AS INT
                                               , @recordsIN    AS INT
                                               , @dataXML      AS XML
...
************************************************************************************************************************************
  ...
  Procedure:  dbo.mc_contact_orgrolesDELETE
     Author:  Chris Carson
    Purpose:  DELETEs dbo.ContactOrgRoles and related coreSHIELD records based on portal view data
    ...
    3)  DELETE temp storage data from dbo.ContactOrgRoles
    4)  DELETE temp storage data from dbo.ContactOrgRoleSystems
...
BEGIN TRY
    ...
    DECLARE @codeBlockDesc01    AS NVARCHAR (128)   = 'Load temp storage with shredded XML data'
          ...
          , @codeBlockDesc03    AS NVARCHAR (128)   = 'DELETE temp storage data from dbo.ContactOrgRoleSystems'
          , @codeBlockDesc04    AS NVARCHAR (128)   = 'DELETE temp storage data from dbo.ContactOrgRoles' ;
...
INNER JOIN  dbo.ContactOrgRoleSystems AS crs ON crs.mc_contact_orgrolesID = src.id AND crs.systemID = src.systemID ;


/**/SELECT  @codeBlockNum   = 3
/**/      , @codeBlockDesc  = @codeBlockDesc03 ; -- DELETE temp storage data from dbo.ContactOrgRoleSystems

    DELETE  dbo.ContactOrgRoleSystems
      FROM  dbo.ContactOrgRoleSystems AS crs
     WHERE  EXISTS ( SELECT 1 FROM @sourceData AS src
                      WHERE src.contactOrgRoleID = crs.ID AND src.systemID = crs.systemID ) ;

    SELECT  @controlCount = @@ROWCOUNT ;
    IF  ( @recordsIN <> @controlCount )
        RAISERROR( @controlTotalsError, 16, 1, 'Records IN', @recordsIN, 'ContactOrgRoleSystems DELETEs', @controlCount ) ;
...
/**/      , @codeBlockDesc  = @codeBlockDesc04 ; -- DELETE temp storage data from dbo.OrgLocations
    ...
    DELETE  dbo.ContactOrgRoles
     ...
     WHERE  EXISTS ( SELECT 1 FROM @sourceData AS src WHERE src.contactOrgRoleID = cor.ID )
                AND
            NOT EXISTS ( SELECT 1 FROM dbo.ContactOrgRoleSystems AS crs WHERE crs.ID = cor.ID ) ;
...
BEGIN CATCH
    SELECT  @errorData = '<b>trigger data passed into coreShield</b></br></br>'
                       + '<table border="1">'
                       + '<tr><th>id</th><th>systemID</th><th>contactOrgRoleID</th></tr>'
                       + CAST ( ( SELECT  td = id, ''
                                        , td = systemID, ''
                                        , td = contactOrgRoleID, ''
                                    FROM  @sourceData
