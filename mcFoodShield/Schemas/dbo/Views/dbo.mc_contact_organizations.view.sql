...
GO
CREATE TRIGGER  mc_contact_organizationsTR
            ON  dbo.mc_contact_organizations
INSTEAD OF INSERT, UPDATE, DELETE
AS
/*
************************************************************************************************************************************

    Trigger:    mc_contact_organizationsTR
     Author:    Chris Carson
...
/**/      , @codeBlockDesc  = @codeBlockDesc03  ; --  EXECUTE coreShield process for INSERTs
    ...
    IF  EXISTS ( SELECT 1 FROM inserted ) AND NOT EXISTS ( SELECT 1 FROM deleted )
        EXECUTE Core.mc_contact_organizationsINSERT  @systemID
                                                   , @recordsIN
                                                   , @dataXML
                                                   , @errorMessage OUTPUT ;
...
/**/      , @codeBlockDesc  = @codeBlockDesc04  ; --  EXECUTE coreShield process for UPDATEs
    ...
    IF  EXISTS ( SELECT 1 FROM inserted ) AND EXISTS ( SELECT 1 FROM deleted )
        EXECUTE Core.mc_contact_organizationsUPDATE  @systemID
                                                   , @recordsIN
                                                   , @dataXML
                                                   , @errorMessage OUTPUT ;
...
/**/      , @codeBlockDesc  = @codeBlockDesc05  ; --  EXECUTE coreShield process for DELETEs
    ...
    IF  NOT EXISTS ( SELECT 1 FROM inserted ) AND EXISTS ( SELECT 1 FROM deleted )
        EXECUTE Core.mc_contact_organizationsDELETE  @systemID
                                                   , @recordsIN
                                                   , @dataXML
                                                   , @errorMessage OUTPUT ;
